defmodule LivedjWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div
        id={"#{@id}-bg"}
        class="bg-zinc-100/50 dark:bg-zinc-900/50 fixed inset-0 transition-opacity"
        aria-hidden="true"
      />
      <div
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed top-2 right-3 w-80 sm:w-96 z-50 rounded-lg p-3 ring-1 cursor-pointer",
        @kind == :info &&
  ...
  @doc """
  Renders a simple form for search use cases.

  ## Examples

      <.add_media_form for={@form} phx-change="validate" phx-submit="save">
        <.input field={@form[:email]} label="Email"/>
        <.input field={@form[:username]} label="Username" />
        <:actions>
          <.button>Save</.button>
  ...
  slot :inner_block, required: true
  slot :field, doc: "the slot for the form input, such as a text input"
  slot :actions, doc: "the slot for form actions, such as a submit button"
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    assigns =
      assign_new(assigns, :checked, fn ->
        HTML.Form.normalize_value("checkbox", value)
      end)
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LivedjWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LivedjWeb.Gettext, "errors", msg, opts)
    end
  end
