defmodule LiveDj.StatsTest do
  ...
  describe "badges" do
    ...
    test "update_badge/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Stats.update_badge(badge, @invalid_attrs)
  ...
  describe "users_badges" do
    ...
    test "list_users_badges/0 returns all users_badges", %{
      badge: badge,
      user: user
    } do
    ...
    test "get_user_badge!/1 returns the user_badge with given id", %{
      badge: badge,
      user: user
    } do
      user_badge = user_badge_fixture(%{badge_id: badge.id, user_id: user.id})
    ...
    test "create_user_badge/1 with valid data creates a user_badge", %{
      badge: badge,
      user: user
    } do
      valid_attrs =
        Enum.into(@valid_attrs, %{badge_id: badge.id, user_id: user.id})

      assert {:ok, %UserBadge{} = _user_badge} =
               Stats.create_user_badge(valid_attrs)
    end

    test "create_user_badge/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} =
               Stats.create_user_badge(@invalid_attrs)
    end

    test "update_user_badge/2 with valid data updates the user_badge", %{
      badge: badge,
      user: user
    } do
    ...
    test "update_user_badge/2 with invalid data returns error changeset", %{
      ...
      assert {:error, %Ecto.Changeset{}} =
               Stats.update_user_badge(user_badge, @invalid_attrs)

      assert user_badge == Stats.get_user_badge!(user_badge.id)
    end

    test "delete_user_badge/1 deletes the user_badge", %{
      badge: badge,
      user: user
    } do
      user_badge = user_badge_fixture(%{user_id: user.id, badge_id: badge.id})
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Stats.get_user_badge!(user_badge.id)
      end
    end

    test "change_user_badge/1 returns a user_badge changeset", %{
      badge: badge,
      user: user
    } do
      user_badge = user_badge_fixture(%{user_id: user.id, badge_id: badge.id})
